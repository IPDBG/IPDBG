...
module wb_ebr_ctrl
  #(parameter SIZE             = 4096,
   ...
   parameter ST_SUBWR = 3'b110;
	 ...
	 wire [EBR_WB_DAT_WIDTH-1:0] data_blockram;         // Data read from RAM
...
//	    );
	  ...
	  #(
	  	.WIDTH(32),
		.ADDR_WIDTH(EBR_WB_ADR_WIDTH),
		.MEM_INIT_FILE(INIT_FILE_NAME)
	  )blockram(
		.clk(CLK_I),
		.write_enable(write_enable),  // Bus [0 : 0]
		.wr_addr(write_address[EBR_WB_ADR_WIDTH-1:0]), // Bus [8 : 0]
		.rd_addr(read_address[EBR_WB_ADR_WIDTH-1:0]), // Bus [8 : 0]
		.wr_data(write_data), // Bus [31 : 0]
		.rd_data(data)); // Bus [31 : 0]
////data_blockram
