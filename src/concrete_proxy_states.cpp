...
void init::toggle(proxy* proxy){
    ...
    if(command == proxy->CMD_START){
        ...
        //0xAA: init -> get_bus_widths
        if(command == proxy->CMD_GET_BUS_WIDTHS){
            //forward command to ipdbg_la
            while(proxy->write_to_client(&command, 1) != 0){}
            ...
            //0xBB: init -> get_la_id
            if(command == proxy->CMD_GET_LA_ID){
                //forward command to ipdbg_la
                while(proxy->write_to_client(&command, 1) != 0){}
                proxy->setState(get_la_id::getInstance());
            }
            else{
                if(command == proxy->CMD_GET_FEATURES){
                    if(proxy->is_old_idbg_la()){
                        //do not forward command to ipdbg_la
                    ...
                    if(command == proxy->CMD_GET_RLC_WIDTH){
                        //forward command to ipdbg_la
                        while(proxy->write_to_client(&command, 1) != 0){}
                        proxy->setState(get_rlc_width::getInstance());
                    }
                    else{
                        if(command == proxy->CMD_GET_CHANNEL_NAMES){
                            proxy->setState(get_channel_names::getInstance());
                        }
                        else{
                            if(command == proxy->CMD_GET_SAMPLE_RATE){
...
void start::toggle(proxy* proxy){
    proxy->setState(init::getInstance()); //return to init
}
...
void get_la_id::enter(proxy* proxy){
    ...
    if(proxy->read_from_client(la_id, buffer_size) == 0){
        int cmp_IDBG = strncmp(reinterpret_cast<const char*>(la_id), proxy->OLD_LA_ID, buffer_size);
        int cmp_idbg = strncmp(reinterpret_cast<const char*>(la_id), proxy->NEW_LA_ID, buffer_size);
        if(!cmp_IDBG && !cmp_idbg){
            //invalid device ID: expected 'idbg' or 'IDBG'
        }
        else{
            if(cmp_IDBG == 0){
                proxy->set_old_idbg_la_flag(true);
            }
            else{
                proxy->set_old_idbg_la_flag(false);
            }
        }
...
void get_features::enter(proxy* proxy){
    if(proxy->is_old_idbg_la()){
        ...
        //do not read features from ipdb_la
...
void get_sample_rate::enter(proxy* proxy){
    ...
    proxy->write_to_server(&buffer[0], buffer_size);
}
